...
#include"Elements.h"
    ...
    void ForwardList::pop_front()//удаляет элемент c начала списка
    {
        Element* New = Head;
        Head = Head->pNext;
        delete New;
    ...
    void  ForwardList::insert(int Data, int index) //вставляет элемент в список по заданному индексу
    {
        if (index == 0)
        {
            push_front(Data);
    ...
    void  ForwardList::erase(int index) 				//удаляет элемент из списка по заданному индекс
    {
        if (index == 0)
        {
            pop_front();
    ...
    void ForwardList::pop_back()//удаляет элемент c конца списка
    {
        Element* Temp = Head;
